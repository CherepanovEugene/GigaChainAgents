...
def orchestrator_agent(data):
    ...
    llm_prompt = (
        f"Ты -аналитик данных высокого уровня"
        f"Проанализируй список объектов: {', '.join(object_list)}\n"
        f"Проанализируй запрос пользователя: {query}\n"
        f"Ответь на вопрос: Есть ли в запросе упоминание одного из объектов в списке? Отвечай только 'Да' или 'Нет'."
    )

    # Логируем отправляемый запрос в LLM
...
if __name__ == "__main__":
    ...
    # Тестовый запрос
    user_id = "user_001"
    user_query = "Какое водоизмещение у системы CRM?"
